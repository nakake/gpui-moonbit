...
アプリの実装パターンは [`app/app.mbt`](app/app.mbt)（Counter）が手本です。3 つの要素から成ります。
...
pub fn build_tree(view : Int) -> Result[Unit, Int] {
  ...
  // クリック可能なボタン: click_id は dispatch にルーティングされる
  cb.div()
...
利用可能なコマンド: `div` / `text` / `set_size` / `set_bg` / `set_flex_row` / `set_flex_col` / `set_center` / `set_gap` / `set_rounded` / `set_on_click` / `set_key` / `set_padding` / `set_border` / `add_child` / `set_root`。色成分は 0–255 にクランプされます。テキストは内部で UTF-8 にエンコードされ、明示長で送られます（NUL 終端なし）。
...
### 3. イベントを受け取る（callback 契約）

Rust からのイベントは、パッケージ `app` の関数 `dispatch` に固定の 5×i32 envelope で届きます。これは ABI 契約であり、パッケージ名と関数名は変えません。
