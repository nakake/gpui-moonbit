...
- 2026-07-15: **テキスト描画の不具合を修正** (詳細は `troubleshooting.md`)
  - String が先頭1文字に切れる問題: MoonBit native の String は UTF-16 非NUL終端。UTF-8+NUL の `Bytes` で渡すよう修正
...
- 2026-07-16: **対話機能 (クリック → 再描画) 完成!** — 実機でクリック挙動まで確認済み
  ...
  - **ビルド自動化** (`build.sh`): MoonBit のマングル名を `nm` で抽出 → `build.rs` が `#[link_name]` extern を生成。改名/マングル方式変更に自動追従(手書きリテラル無し)
...
- 2026-07-16: **キーイベント対応 完成!** — k / j / r で +1 / -1 / reset(実機確認済み)
  ...
  - **キーボードには `.app` バンドルが必要**: 非バンドルのターミナル起動バイナリには macOS がキーボードを配送しない(マウスは届く)。`bundle.sh` で最小バンドル化 → `open dist/Counter.app` or 直接実行
