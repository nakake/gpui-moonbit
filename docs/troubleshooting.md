...
### 修正
`moonbit-bindings/` から `WAYLAND_DISPLAY` を unset して起動し、X11 (XWayland)
バックエンドを使う。
...
Wayland 初期化失敗時の X11 フォールバックは panic safety と合わせて対応するため、
現時点では実装せず [issue #19](https://github.com/nakake/gpui-moonbit/issues/19) / [issue #2](https://github.com/nakake/gpui-moonbit/issues/2) に延期している。
