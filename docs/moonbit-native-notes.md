...
> **前提**: すべて `preferred_target = "native"`。§1〜§8 は主に macOS(arm64 / Mach-O)を基準にした観測で、§9/§10 は後から追加した Linux/Windows の観測である。
...
> **現行設計 (2026-07-19)**: コールバック ABI は `app.dispatch(kind, id, a, b)` の 4 個の `i32` に固定され、build driver が自動追従するのはその実マングル表記だけである。関数名・パッケージ名・引数数・型の変更は自動追従の対象ではなく、両 build driver と ABI 方針を更新して検証する。
