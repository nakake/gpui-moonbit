...
- **Linux のリンク列**(`cargo rustc -- --print native-static-libs`, gpui 0.2.2): `-lstdc++ -lxcb -ldl -lxkbcommon -lxkbcommon-x11 -lgcc_s -lutil -lrt -lpthread -lm -lc`。
  ...
  - 観測日: 2026-07-18
- **WSLg の制約**:
  - Wayland バックエンドは WSLg のコンポジタで `UnsupportedVersion` panic(gpui-0.2.2 `wayland/client.rs:151`、プロトコルバージョン不足)。`WAYLAND_DISPLAY` を unset して X11(XWayland)に落とすと動く。
  - この panic は `extern "C"` の `gpui_run_window` を越えられず abort する(= C export の catch_unwind 化が必要という P0 指摘の実例)。
...
- **cc-link-flags は `/link` プレフィックス必須**: moon はユーザーフラグの**後ろ**に自前の `/link /LIBPATH:$MOON_HOME/lib` を付ける。ユーザー側が `/link` を書かないと `/LIBPATH:` が cl のコンパイルオプションとして無視される(D9002)。ユーザーが `/link` を書くと moon 側の後続 `/link` が LNK4044 警告になるが無害。
- **リンクに必要な追加 LIBPATH**(build.ps1 が自動収集):
  1. `cargo rustc -- --print native-static-libs` のライブラリ列(user32.lib, d3d系, `/defaultlib:msvcrt` 等)
  2. **gpui の build.rs が生成する `gpui.lib`**: `target\debug\build\gpui-<hash>\out\`(macOS/Linux には無い Windows 固有の産物)
  3. **windows-rs のインポートライブラリ** `windows.0.5x.0.lib`: cargo レジストリ内 `windows_x86_64_msvc-*/lib/`
- **CRT 混在警告 LNK4098**(LIBCMT vs msvcrt)が出るがリンク・実行とも成功。厳密には moon の cl 呼び出し(/MT系)と Rust(/MD系)の CRT を揃えるべき(未対応)。
- **動作確認済み**(2026-07-18): DirectX 11 レンダリングでウィンドウ表示、クリック×3(+1,+1,+10)とキー `k` で Count: 13 をスクリーンショット検証。**キーボードは素の exe で動く**(macOS の .app バンドル要件のようなものは無い)。
- 既知の見た目の問題: コンテンツがウィンドウ全体を埋めず、下部が**透過**する(Linux では黒帯、Windows では背後が透ける。ルートレイアウトが h_full でない疑い、アプリ側課題)。
- 観測日: 2026-07-18
