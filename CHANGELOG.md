...
- `cmd/main` / `cmd/roundtrip` を、per-OS の `moon.pkg` テンプレート（`moon.pkg.macos` / `.linux` / `.windows`）と `cc-link-flags` のプレースホルダ置換で組み立てる方式から、**tracked な `moon.pkg` + `nakake/gpui-bindings/link` の import** 方式へ移行した（#126、RFC 0005 D5/D6）。リンクフラグの供給源は prebuild（`moonbit-bindings/build.py`）が出力する LinkConfig の一本だけになり、examples・`tests/consumer` と同じ消費者経路を cmd も通る。`cc-flags` / `cc-link-flags` は cmd から全廃し、テンプレート 6 ファイルと `build.sh` の `write_moon_pkg` / `build.ps1` の `Write-MoonPkg`、`.gitignore` の cmd `moon.pkg` 2 行を削除した。コールバックのマングルシンボルは `build.py` が `abi.toml` の `[callback]` から決定的に計算して `gpui-sys/mb_symbol.txt` に書く形へ一本化し（既存ファイルは上書きせず、値が食い違う場合だけ警告 = 手動 override は温存）、build driver は生成 C からの抽出をやめて**事後検証**（最終実行ファイルにシンボルが exactly-once + 生成 C の C プロトタイプと `[callback] params` の照合）と、リンク失敗時の実シンボル候補の提示に役割を変えた。段構成は Preflight → [0/4] codegen → [1/4] `moon check` → [2/4] cmd exe の削除 + `moon build` → [3/4] 事後検証 → [4/4] roundtrip →（macOS）bundle。
...
- `cmd/main` を最小ランナー化した（#125、RFC 0004 §4）。Counter デモが `examples/counter` へ出たので、ここに残るのは build driver が必要とする最小の実行ファイル（リンクと事後検証の対象）である。スタブではなくクリックとキーで状態が動く実アプリのままにしてあり、イベント経路が壊れたときに最初に気付ける。
...
- #125 の docs 同期（RFC 0004 §7-5）から漏れていた記述を修正した。`moonbit-bindings/README.mbt.md` の callback 契約節が「パッケージ `app` の関数 `dispatch` に固定。パッケージ名と関数名は変えません」という廃止済みの旧仕様を現在形で述べていたのを、`dispatch_entry` + `register_dispatch` の登録モデル（RFC 0004）に書き換え。あわせて同節に `EVENT_SCROLL`（= 8、#89）とコマンド一覧の `set_scroll_id` を追記。`docs/architecture.md` の存在しない `app.mbt` への言及 2 箇所（実体は `examples/counter/counter/counter.mbt` と `gpui-bindings-ffi.mbt`）と `dist/Counter.app` 2 箇所（現 `dist/Runner.app`）、`docs/architecture.html` の同種の残骸 4 箇所とイベント種別（5〜8 欠落）・status code 表（-11〜-16 欠落）、`docs/roadmap.md` の RFC 一覧（0004 欠落）、`docs/rfc/0004` のステータス表記と §8 未決事項 3・4（`_keep` 撤廃は検証済み、cmd 再設計は #126 起票済み）も現状へ同期した。registry 消費の残件は #132 として起票。
